...
def execution_timer(agent_name: str):
    ...
    try:
        ...
        logger.error(
            f"Error during {agent_name} execution: {e} - Execution time: {execution_time:.2f} seconds",
            exc_info=True,
        )
        ...
        execution_time = end_time - start_time
        logger.info(
            f"{agent_name} finished successfully - Execution time: {execution_time:.2f} seconds"
        )
