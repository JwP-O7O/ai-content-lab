...
class AbTestingAgent:
    ...
    async def run_experiment(self):
        try:
            ...
            self.logger.info(f"Agent {self.agent_id}: Experiment completed in {execution_time:.2f} seconds. Results: {results}")
            return results
        except Exception as e:
            self.logger.error(f"Agent {self.agent_id}: An error occurred: {e}", exc_info=True)
            return None

    async def _execute_experiment(self):
        try:
            # Simulate fetching data
            data = await self.data_source.get_data(self.experiment_config.get("data_source_params",{}))
            ...
            self.logger.error(f"Agent {self.agent_id}: Error during experiment execution: {e}", exc_info=True)
            raise


    async def _process_variant(self, data, variant_name):
        try:
            self.logger.info(f"Agent {self.agent_id}: Processing variant {variant_name} with {len(data)} items")
            ...
            self.logger.info(f"Agent {self.agent_id}: Variant {variant_name} result: {result}")
            return result
        except ZeroDivisionError:
            self.logger.warning(f"Agent {self.agent_id}: Variant {variant_name} has no data, result is zero.")
            return 0
        except Exception as e:
            self.logger.error(f"Agent {self.agent_id}: Error processing variant {variant_name}: {e}", exc_info=True)
            raise
...
async def main():
    ...
    agent1 = AbTestingAgent(agent_id="agent_001", data_source=data_source, experiment_config=experiment_config)
    agent2 = AbTestingAgent(agent_id="agent_002", data_source=data_source, experiment_config=experiment_config)
