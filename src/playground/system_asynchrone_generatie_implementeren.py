...
class LLMClient:
    ...
    async def _make_api_call(self, prompt: str) -> str:
        ...
        try:
            ...
            await asyncio.sleep(2) # Simuleer wachttijd
            if "error" in prompt.lower():
                raise ValueError("Simulated API error!")
            response = f"Simuleerde LLM generatie voor: {prompt}"
            self.logger.info(f"Ontvangen response: {response}")
            return response
        except ValueError as e:
            self.logger.error(f"API call error: {e}")
            raise # Her-raise de exception
    ...
    async def generate(self, prompt: str) -> str:
        ...
        try:
            ...
            end_time = time.time()
            self.logger.info(f"Generatie voltooid in {end_time - start_time:.2f} seconden")
            return response
        except Exception as e:
            self.logger.error(f"Fout tijdens generatie: {e}")
            return "Er is een fout opgetreden tijdens de generatie." # Foutafhandeling
