...
class AIService:
        ...
        try:
            ...
            for model_name in self.model_candidates:
                try:
                    ...
                    logger.info(
                        f"🧠 AI Verbonden | Model: {model_name} | Key: {masked_key}"
                    )
                    return
                except Exception:
                    continue
    ...
    @backoff.on_exception(
        ...
        (
            Exception,
        ),  # Catch all exceptions (of specifiekere, zoals 'google.api_core.exceptions.ResourceExhausted')
        max_tries=3,  # Maximaal aantal pogingen
        on_backoff=lambda details: logger.warning(
            f"🔄  Retry in {details['wait']:0.1f}s, poging {details['tries']} van 3..."
        ),
    )  # Loggen van retries
