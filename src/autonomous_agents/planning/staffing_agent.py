...
class StaffingAgent:
    ...
    async def evaluate_team_needs(self):
        ...
        try:
            ...
            # Extra validatie
            if not all(key in response_json for key in ["rol", "bestandsnaam", "beschrijving", "code"]):
                raise ValueError("JSON response is incomplete")

            #  Check of de rol specifiek genoeg is (simpele versie)
            if "specifiek" not in response_json["beschrijving"].lower() and "taak" not in response_json["beschrijving"].lower():
                 logger.warning(f"[{self.name}] De agent rol lijkt niet specifiek genoeg. Response: {response_json['beschrijving']}")
            ...
            if not filename or not code_part:  # Check of filename en code leeg zijn
                logger.info(f"[{self.name}] Team is compleet voor nu (of er ging iets fout met de response).")
                return
            ...
            try:
                module_name = filename.replace(".py", "").replace("/", ".")  # Aanpassen voor path
                spec = importlib.util.spec_from_file_location(module_name, filepath)
                module = importlib.util.module_from_spec(spec)
                spec.loader.exec_module(module)
                logger.info(f"[{self.name}] Code validatie geslaagd: {filename}")
            except Exception as e:
                logger.error(f"[{self.name}] Code validatie mislukt voor {filename}: {e}")
            ...
            logger.error(f"[{self.name}] Geboorte mislukt (fout met parsing JSON response): {e}\nResponse: {response_text}")
        except Exception as e:
