...
class SecurityGuard:
    ...
    def __init__(self, system_files: List[str] = None, allowed_network_domains: List[str] = None, data_access_logs: List[Dict[str, Any]] = None):
        """
        ...
        self.allowed_network_domains = allowed_network_domains if allowed_network_domains is not None else []
        ...
        self.monitoring_thread = threading.Thread(target=self.start_monitoring, daemon=True) # Run in daemon mode
    ...
    def monitor_system(self):
        ...
        for file in self.system_files:
            try:
                ...
                time.sleep(0.1) # Debounce: short sleep to allow potential operations to complete.
    ...
    def monitor_network_activity(self):
        ...
        try:
            ...
            with urllib.request.urlopen("https://api.ipify.org/") as response: # Simulate network traffic (Get own IP)
                ip = response.read().decode("utf-8")
                if ip:
                    try:
                        domain = socket.gethostbyaddr(ip)[0]
                        if not any(domain.endswith(allowed_domain) for allowed_domain in self.allowed_network_domains):
                            self.report_incident(f"Netwerkverkeer naar onbekend domein: {domain} ({ip})")
        ...
        for log_entry in self.data_access_logs:
            try:
                ...
                if "gevoelige_data" in str(log_entry.get("data", "")):
                    self.report_incident(f"Ongeautoriseerde toegang tot gevoelige data: {log_entry}")

                #Voorbeeld:  Detecting suspicious amounts of data access.
                if log_entry.get("type") == "read" and log_entry.get("bytes_accessed", 0) > 1000000: # 1MB example
                ...
                if "prompt" in log_entry and re.search(r"(?i)(drop table|delete from)", log_entry["prompt"]):
                    self.report_incident(f"Mogelijke prompt injection detected: {log_entry}")


            except Exception as e:
                logging.error(f"Fout bij het analyseren van log entry: {log_entry}. Fout: {e}")
...
if __name__ == '__main__':
    ...
    example_logs = [
        {"timestamp": "2024-10-27T10:00:00", "user": "test_user", "action": "read", "file": "gevoelige_data.txt", "data": "Geheime informatie"},
        {"timestamp": "2024-10-27T10:01:00", "user": "test_user", "action": "read", "file": "some_file.txt", "data": "Some regular data"},
        {"timestamp": "2024-10-27T10:02:00", "user": "attacker", "action": "read", "file": "config.py", "data": "Some more data"},
        {"timestamp": "2024-10-27T10:03:00", "user": "malicious_user", "action": "prompt", "prompt": "drop table users; -- comment"}
    ]

    security_guard = SecurityGuard(system_files=critical_files, allowed_network_domains=allowed_domains, data_access_logs=example_logs)
