...
class DeepDebugger:
    ...
    async def fix_broken_code(self, filepath, error_log):
        ...
        try:
            ...
            logger.error(
                f"[{self.name}] ❌ Geen toestemming om bestand te lezen/schrijven: {filepath}"
            )
            return {"status": "failed", "error": "PermissionError", "file": filepath}

        except OSError as e:
            logger.error(
                f"[{self.name}] ❌ OS Error bij lezen/schrijven bestand {filepath}: {e}"
            )
            return {
                "status": "failed",
                "error": "OSError",
            ...
            logger.error(
                f"[{self.name}] ❌ Debugger faalde door een onbekende reden: {e}\nTraceback:\n{traceback.format_exc()}"
            )
            return {
                "status": "failed",
                "error": "UnknownError",
