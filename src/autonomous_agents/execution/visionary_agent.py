        ...
        for attempt in range(self.max_retries + 1):
            try:
                response = requests.get(
                    url, stream=True
                )  # Gebruik stream=True om de download efficienter te maken
                if response.status_code == 200:
                    # Bestandsnaam genereren
                    timestamp = int(time.time())
                    clean_hint = "".join(
                        [c for c in filename_hint if c.isalnum() or c == "_"]
                    )[:15]
                    ...
                    with open(filepath, "wb") as f:
                        for chunk in response.iter_content(
                            chunk_size=8192
                        ):  # Chunk downloaden voor efficientie
                            f.write(chunk)
                    ...
                    logger.warning(
                        f"Fout bij downloaden (poging {attempt + 1}/{self.max_retries + 1}): {response.status_code}"
                    )
                    if attempt < self.max_retries:
                        time.sleep(self.retry_delay)  # Wacht voor het opnieuw proberen
                    else:
                        logger.error("Alle pogingen mislukt.")
                        return {
                            "status": "error",
                            "error": f"Download mislukt met status code: {response.status_code}",
                        }
            except requests.exceptions.RequestException as e:
                logger.error(
                    f"Visionary download error (poging {attempt + 1}/{self.max_retries + 1}): {e}"
                )
                if attempt < self.max_retries:
                ...
                logger.error(
                    f"Visionary error (poging {attempt + 1}/{self.max_retries + 1}): {e}"
                )
                if attempt < self.max_retries:
                    time.sleep(self.retry_delay)
                else:
