...
class GitHubListener:
    ...
    def _validate_config(self):
        ...
        if not self.token:
            logger.warning(f"[{self.name}] GITHUB_TOKEN is not set. GitHub functionality will be severely limited.")
        if not self.repo_name:
            logger.warning(f"[{self.name}] REPO_NAME is not set.  GitHub functionality will be severely limited.")
    ...
    def _get_github_instance(self):
        ...
        if self.github_instance is None:
            if not self.token:
                logger.error(f"[{self.name}] No GitHub token available. Cannot initialize Github instance.")
                return None
            try:
        ...
        try:
            ...
            for issue in open_issues:
                ...
                try:
                    ...
                    logger.warning(f"[{self.name}] Failed to update GitHub issue status (non-critical): {update_err}")

                tasks.append({
                    "title": issue.title.replace("🤖 [WIP]", "").strip(),  # Clean title for task processing
                    "body": issue.body,
                    "issue_obj": issue,
                })
